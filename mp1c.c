...
int main() {
    ...
    char* start_grid = malloc(70);
    final_grid = malloc(70);
    char pieceAscii[20];
    ...
    for(int row = 4; row < 10; row++) {
        ...
        // Read a line
        fgets(start_grid + (GRID_COLS * row), GRID_COLS, stdin);
        // Then read the next character but ignore it
        fgetc(stdin);
    }

    // do the same for final_grid
    for(int row = 4; row < 10; row++) {
        fgets(final_grid + (GRID_COLS * row), GRID_COLS, stdin);
        fgetc(stdin);
    }
    ...
    for(int i = 0; i < numPieces; i++) {
        for(int row = 0; row < 4; row++) {
            fgets(pieceAscii + (PIECE_COLS * row), PIECE_COLS, stdin);
            fgetc(stdin);
        }
...
char* drop_piece_in_grid(char grid[70], char piece[8], int yOffset, char* isSuccess) {
    char* gridCopy = malloc(70);
    copy(grid, gridCopy, 70);
    int maxY = 100;
